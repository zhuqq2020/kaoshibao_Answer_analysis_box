...
(function () {
    ...
    function applyCleanUI() {
        ...
        style.textContent = `
            ...
            /* 确保题目容器有足够的上边距 */
            .question-content, .exam-question, .question-box {
        ...
        // 额外调整：直接修改导航栏的position属性
        setTimeout(() => {
            const header = document.querySelector('.el-header.clearfix');
            if (header) {
    ...
    function unlockVIP() {
        ...
        selectorsToRemove.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => {
                // 确保不删除导航栏相关元素
                if (!el.closest('.el-header') && !el.closest('header')) {
                    el.remove();
                }
            });
        });

        // 移除"开通VIP查看完整解析"相关元素（不包含导航栏）
        ...
        for (let i = 0; i < vipNodes.snapshotLength; i++) {
            ...
            const target = node.closest('button') || node.closest('.el-button') ||
                           ...
                           node.parentElement;
            if (target && !target.classList.contains('app-main') &&
                !target.classList.contains('answer-analysis') &&
                !target.closest('.el-header') && !target.closest('header')) {
                target.remove();
            } else if (!node.closest('.el-header') && !node.closest('header')) {
                node.remove();
            }
        }
        ...
        for (let i = 0; i < deepNodes.snapshotLength; i++) {
            ...
            if (wrapper && !wrapper.classList.contains('app-main') &&
                !wrapper.closest('.el-header') && !wrapper.closest('header')) {
                wrapper.remove();
            }
        }

        // 清理绝对定位的 VIP 图标（不包含导航栏）
        document.querySelectorAll('i, img, svg').forEach(icon => {
            ...
            if (parentText.includes('VIP') || parentText.includes('解析')) {
                ...
                if ((style.position === 'absolute' || style.position === 'fixed') &&
                    !icon.closest('.el-header') && !icon.closest('header')) {
                    icon.remove();
                }
            }
        });
        ...
        setTimeout(() => {
            document.querySelectorAll('[style*="blur"], [style*="opacity"]').forEach(el => {
                if ((el.style.opacity === '0' || el.style.filter?.includes('blur')) &&
                    !el.closest('.el-header') && !el.closest('header')) {
                    el.remove();
                }
            });
        }, 300);
    ...
    function createSettingsUI() {
        ...
        // 构建面板 HTML
        let html = `<div class="ksb-title">⚡ 考试宝助手 v2.2.2 设置</div>`;
        ...
        html += renderSwitch('cleanUI', '🧹 界面净化 (修复导航栏遮挡)');
